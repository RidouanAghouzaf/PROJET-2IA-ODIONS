...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('id', id)
      .maybeSingle();// safe because ID is unique
...
router.post('/', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert([tenantData])
      .select()
      .maybeSingle();// safe: inserting one row

    if (error) throw error;

    res.status(201).json({ message: 'Tenant created successfully', tenant: data });
  } catch (error) {
    console.error('Create tenant error:', error);
    res.status(500).json({ error: { message: 'Failed to create tenant', status: 500 } });
  }
...
router.put('/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq('id', id)
      .select()
      .maybeSingle(); // safe: updating by unique ID
